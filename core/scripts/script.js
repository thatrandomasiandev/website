...
function printResume() {
    ...
    const resumeWindow = window.open('resume.html', '_blank', 'width=800,height=600');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Resume actions dropdown functionality
    const resumeDropdowns = document.querySelectorAll('.resume-actions .dropdown');
...
function openResumeByDomain() {
    const emailInput = document.getElementById('emailDomain');
    const domain = emailInput.value.trim().toLowerCase();
    
    if (!domain) {
        alert('Please enter a work email domain (e.g., nasa.gov)');
        return;
    }
    
    // Determine which resume to open based on domain
    let resumeUrl = 'resume.html'; // Default to regular resume
    let companyType = 'Regular Company';
    
    // NASA and aerospace companies (NASA format)
    if (domain.includes('nasa.gov') || 
        domain.includes('spacex.com') || 
        domain.includes('blueorigin.com') || 
        domain.includes('boeing.com') || 
        domain.includes('lockheed.com') ||
        domain.includes('northrop.com') ||
        domain.includes('raytheon.com') ||
        domain.includes('aerospace.org') ||
        domain.includes('sierra-nevada.com') ||
        domain.includes('virgingalactic.com') ||
        domain.includes('planet.com') ||
        domain.includes('maxar.com')) {
        resumeUrl = 'nasa_resume.html';
        companyType = 'Aerospace/NASA';
    }
    
    // Government agencies (NASA format)
    else if (domain.includes('.gov') || 
             domain.includes('.mil') ||
             domain.includes('defense.gov') ||
             domain.includes('dod.gov') ||
             domain.includes('energy.gov') ||
             domain.includes('nsf.gov') ||
             domain.includes('nih.gov') ||
             domain.includes('darpa.gov')) {
        resumeUrl = 'nasa_resume.html';
        companyType = 'Government Agency';
    }
    
    // Defense contractors (NASA format)
    else if (domain.includes('general-dynamics.com') ||
             domain.includes('l3harris.com') ||
             domain.includes('bae-systems.com') ||
             domain.includes('saic.com') ||
             domain.includes('caci.com') ||
             domain.includes('boozallen.com') ||
             domain.includes('deloitte.com') ||
             domain.includes('accenture.com')) {
        resumeUrl = 'nasa_resume.html';
        companyType = 'Defense Contractor';
    }
    
    // Financial services (usually prefer regular format but some are formal)
    else if (domain.includes('goldman.com') ||
             domain.includes('morganstanley.com') ||
             domain.includes('jpmorgan.com') ||
             domain.includes('bankofamerica.com') ||
             domain.includes('wellsfargo.com') ||
             domain.includes('citigroup.com') ||
             domain.includes('blackrock.com') ||
             domain.includes('fidelity.com')) {
        resumeUrl = 'resume.html';
        companyType = 'Financial Services';
    }
    
    // Consulting firms (usually prefer regular format)
    else if (domain.includes('mckinsey.com') ||
             domain.includes('bain.com') ||
             domain.includes('bcg.com') ||
             domain.includes('pwc.com') ||
             domain.includes('ey.com') ||
             domain.includes('kpmg.com') ||
             domain.includes('deloitte.com')) {
        resumeUrl = 'resume.html';
    ...
    // AI/ML Research Companies (NASA format for research focus)
             ...
             domain.includes('intel.com') ||
             domain.includes('amd.com') ||
             domain.includes('qualcomm.com')) {
        resumeUrl = 'nasa_resume.html';
        companyType = 'AI/ML Research';
    }
    
    // Tech companies (usually prefer regular format)
    else if (domain.includes('amazon.com') ||
             domain.includes('meta.com') ||
             domain.includes('netflix.com') ||
             domain.includes('spotify.com') ||
             domain.includes('uber.com') ||
             domain.includes('lyft.com') ||
             domain.includes('airbnb.com') ||
             domain.includes('stripe.com') ||
             domain.includes('square.com') ||
             domain.includes('palantir.com') ||
             domain.includes('databricks.com') ||
             domain.includes('snowflake.com')) {
        resumeUrl = 'resume.html';
        companyType = 'Technology';
    }
    
    // Healthcare/Pharma (usually prefer regular format)
    else if (domain.includes('pfizer.com') ||
             domain.includes('moderna.com') ||
             domain.includes('johnsonandjohnson.com') ||
             domain.includes('merck.com') ||
             domain.includes('gilead.com') ||
             domain.includes('biogen.com') ||
             domain.includes('amgen.com')) {
        resumeUrl = 'resume.html';
        companyType = 'Healthcare/Pharma';
    }
    
    // Automotive (usually prefer regular format)
    else if (domain.includes('tesla.com') ||
             domain.includes('ford.com') ||
             domain.includes('gm.com') ||
             domain.includes('chrysler.com') ||
             domain.includes('toyota.com') ||
             domain.includes('honda.com') ||
             domain.includes('bmw.com') ||
             domain.includes('mercedes-benz.com')) {
        resumeUrl = 'resume.html';
        companyType = 'Automotive';
    }
    
    // Open the appropriate resume
    window.open(resumeUrl, '_blank');
    
    // Show confirmation of which format was selected
    console.log(`Selected ${companyType} format for ${domain}`);
    
    closeResumeModal();
}

function openRegularResume() {
...
function goToStandardResume() {
    // Open the standard resume in a new tab
    window.open('resume.html', '_blank');
    closeFeaturesPopup();
}

// Show features popup when page loads (after a short delay) - ONLY ONCE per session
document.addEventListener('DOMContentLoaded', () => {
    // Check if user has already seen the popup this session
    const hasSeenPopup = localStorage.getItem('featuresPopupShown');
    
    if (!hasSeenPopup) {
        // Show the features popup after 2 seconds
        setTimeout(() => {
            showFeaturesPopup();
            // Mark that the user has seen the popup
            localStorage.setItem('featuresPopupShown', 'true');
        }, 2000);
    }
});

// Close features popup when clicking outside
document.addEventListener('click', (e) => {
    const popup = document.getElementById('featuresPopup');
...
// Close features popup on escape key
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        closeFeaturesPopup();
    }
